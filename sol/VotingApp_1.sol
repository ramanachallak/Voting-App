...
contract VotingAppCrowdsale {
    ...
    function setStartTime(uint start_time) {
        require(msg.sender ==_owner, "Only Voting Authority can set Start Time");
        _start_time = start_time;
    }

    function setEndTime(uint end _time) {
        require(msg. sender ==_owner, "Only Voting Authority can set Stop Time");
        _end_time = end_time;
    }
    function setEndTime(uint end_time) {
    _end_time = end_time;
    }
    //function isOpen) public returns(bool)
    //return now<_end_time && now>_start_date;
    function getStartTime() public returns(uint)( return _start_time;)
    function getCloseTime() public returns(uint){
    return _end_time;
} 
